...
export async function getSignalsForRun(tickers, runDate) {
  const Keys = tickers.map((t) => ({
    pk: `TICKER#${t}`,
    sk: `RUN#${runDate}`,
  }));
  let requestItems = { [TABLE_SIGNALS]: { Keys } };
  const items = [];

  // Retry loop to handle UnprocessedKeys from BatchGet
  do {
    const res = await ddb.send(
      new BatchGetCommand({ RequestItems: requestItems }),
    );
    const batch = res.Responses?.[TABLE_SIGNALS] ?? [];
    items.push(...batch);
    const unprocessed = res.UnprocessedKeys?.[TABLE_SIGNALS];
    requestItems = unprocessed ? { [TABLE_SIGNALS]: unprocessed } : null;
  } while (requestItems);

  const map = new Map(items.map((i) => [i.ticker, i]));
  return tickers.map(
    (t) => map.get(t) ?? { ticker: t, runDate, missing: true },
  );
}
